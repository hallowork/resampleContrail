...
#SBATCH --job-name=contrail_process
#SBATCH --array=0-364                    # 处理一年的数据 (0-364 = 365天)
#SBATCH --output=logs/contrail_%A_%a.out # 输出文件
#SBATCH --error=logs/contrail_%A_%a.err  # 错误文件
#SBATCH --mem-per-cpu=${MEM_PER_CPU}G    # 每CPU核心的内存
#SBATCH --cpus-per-task=${WORKERS_PER_TASK} # CPU核心数（与worker数量一致）
#SBATCH --time=${TIME_LIMIT}             # 最大运行时间
#SBATCH --mail-type=FAIL,END             # 任务失败或结束时发送邮件
